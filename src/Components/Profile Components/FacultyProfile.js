...
import { Stepper, Button, TextInput, Box, MantineProvider, Text } from "@mantine/core";
	...
	return (
		<> 
			...
			<Stepper
				...
				size="xs"
				color="dark-teal"
				style= {{margin: "1rem 1rem 1rem 15rem"}}
			>
				<Stepper.Step label="Basic Details" allowStepSelect={active > 0}>
					<div>
						<h2 style= {{fontSize: "1.5rem", marginTop: "1rem"}}>Basic Details</h2>
					</div>
					<Box
				sx={(theme) => ({
					backgroundColor:
			...
			>
					...
					>
						Upload your image <p style= {{color: "rgb(240,62,62)", display: "inline"}} >*</p>
					</Text>
					<UploadDelete />
					</div>
					</Box>
				...
				<Stepper.Step label="Social Media Links" allowStepSelect={active > 6}>
					<div>
						<h2 style= {{fontSize: "1.5rem", marginTop: "1rem"}}>Social Media Links</h2>
					</div>
					<Box
				sx={(theme) => ({
					backgroundColor:
			...
			<div className="flex-container">
				...
				{ active < 8 ? <div> <Button variant="filled" color="dark-teal" onClick={nextStep}> Next step
					</Button> </div> : <br /> }
